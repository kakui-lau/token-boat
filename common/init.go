...
func printHelp() {
	...
	fmt.Println("Maintainer: QuantumNous - http://tokenboat.com")
...
func InitEnv() {
	...
	AuthRefreshRateLimitEnable = GetEnvOrDefaultBool("AUTH_REFRESH_RATE_LIMIT_ENABLE", true)
	AuthRefreshRateLimitNum = GetEnvOrDefault("AUTH_REFRESH_RATE_LIMIT", 120)
	AuthRefreshRateLimitDuration = int64(GetEnvOrDefault("AUTH_REFRESH_RATE_LIMIT_DURATION", 60))
...
func initConstantEnv() {
	constant.StreamingTimeout = GetEnvOrDefault("STREAMING_TIMEOUT", 3600)
	...
	// 是否强制记录日志里的客户端 IP（覆盖每个用户的 RecordIpLog 开关），默认开启
	constant.AlwaysRecordIp = GetEnvOrDefaultBool("ALWAYS_RECORD_IP", true)
	// 任务轮询时查询的最大数量
