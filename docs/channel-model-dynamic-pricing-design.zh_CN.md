...
- 新建表；
- 当前模型价格导入官方价格版本；
- 对导入价格标记 `source=legacy_import`，由财务确认它确实可作为官方基准，不能仅因名称叫 ModelRatio 就自动认定为官方牌价；
- 由 channels + abilities 生成 channel_models；
- 默认采购折扣 1.0；
- 影子计算。
...
| 模块 | 规模 |
|---|---:|
| 数据模型和迁移 | 1,500–2,500 行 |
| 定价和版本引擎 | 2,500–4,000 行 |
| 路由、重试和熔断 | 3,000–5,000 行 |
| 结算、日志和对账 | 2,000–3,500 行 |
| 管理 API | 1,500–2,500 行 |
| 前端 | 4,000–6,000 行 |
| 测试 | 5,000–8,000 行 |

完整版本建议 3–4 名开发、1 名测试，并由财务和商务参与，周期约 12–16 周。

第一期只实现渠道模型、采购价、销售价和动态结算，预计 6–8 周。
