...
- 已建立不可变运行时价格目录，按 `group + model` 聚合完整候选价格束；目录定时刷新，单个模型价格链无效时仅停止该模型，不允许回退旧路由或旧计费。
...
- 已移除用户、模型、分组和流量百分比灰度。完整价格链的全部候选渠道启用 V2 后立即接管真实路由与收费；管理接口不再允许把已启用模型切回 legacy。
...
- 请求失败时，带价格审计回调的退款会在返回响应前同步完成退款结果和快照终态写入，避免进程重启造成“额度已退但快照仍待确认”；没有发生预扣的失败请求直接归档为 `refunded`，只有退款或结算实际失败才保留为 `pending`。
- Root 管理员核对额度流水和请求日志后，可通过 `POST /api/pricing-admin/request-pricing-snapshots/:id/confirm-refunded` 将历史 `pending` 快照归档为 `refunded`；该操作只确认已有退款事实，不再次执行退款，并记录操作者、请求和用户的管理审计。
- 价格快照保存 `resolution`、`resolved_at` 和 `resolved_by`，区分系统自动退款与管理员人工确认，并保留处理时间和操作人。
- 待确认快照同时保存结构化 `failure_code` 和长度受限的 `failure_reason`；退款、计费结算、任务结算、用量解析和价格表达式计算失败会写入对应原因，价格对账表可直接查看，不再必须先搜索服务日志。
- 价格对账面板会汇总待处理异常、超过 15 分钟的预留、近 24 小时成功结算与退款数量，并标出最早异常时间；汇总只使用跨 SQLite、MySQL 和 PostgreSQL 一致的状态与时间条件，不依赖数据库专有聚合函数。
- 价格对账异常列表支持按请求 ID、模型或渠道关键词搜索及服务端分页，异常超过 20 条时可逐页检查，不再只展示首屏记录；每条异常直接展示请求冻结的采购成本、销售金额及币种，CSV 导出会继承当前关键词，管理员不需要先导出全量文件才能定位并核对金额。
- 价格对账页可在“仅看异常”和“全部价格记录”之间切换，完整查看 `reserved`、`pending`、`settled` 和 `refunded` 历史；分页、关键词和 CSV 导出始终继承当前视图。
- 全部价格记录支持按快照状态和计费模式组合筛选；筛选变化会回到第一页，列表请求和 CSV 导出使用完全相同的查询条件。
- 列表和 CSV 还支持按快照创建时间起止范围筛选，后端使用 Unix 秒边界，便于按事故窗口或财务账期导出。
- 每条价格快照可展开详情，查看请求 ID、采购/销售价格版本、处理结果以及格式化后的预估用量和实际用量，便于核对冻结契约与最终结算输入。
- Root 管理员可把当前对账筛选结果导出为 CSV，单次最多 10,000 条，包含冻结的采购/销售金额、预扣/结算额度及状态时间；文本列会防护电子表格公式注入，避免请求 ID、模型或渠道名称被 Excel 等工具当作公式执行。
- 独立的熔断分析页展示渠道名称、连续失败、成功率、平均延迟、冷却截止时间、限流、半开探测和恢复，并支持按渠道和事件类型筛选；配置 Redis 后，状态、指标和最近 200 条事件通过原子脚本在所有实例间共享，适用于 EKS 多副本部署。Amazon ElastiCache 当前必须使用关闭 Cluster Mode 的复制组主端点；项目的 Redis 客户端不是 Cluster Client。未配置 Redis 时页面明确标识仅为单实例状态。Root 管理员可在二次确认后手动清除共享熔断状态，并写入管理审计和熔断事件。
- 运行时路由按采购成本、成功率、平均延迟和渠道模型质量权重计算综合评分，默认权重分别为 50%、25%、15% 和 10%。成功率使用贝叶斯先验避免冷启动样本抖动，延迟使用 EWMA 平滑；熔断状态仍先于评分执行硬过滤。四项权重可通过 `PRICING_ROUTE_COST_WEIGHT`、`PRICING_ROUTE_SUCCESS_WEIGHT`、`PRICING_ROUTE_LATENCY_WEIGHT` 和 `PRICING_ROUTE_QUALITY_WEIGHT` 配置，运行时会归一化，全部为零时恢复安全默认值。
- Token 阶梯价和多模态阶梯/条件价均已提供结构化可视化编辑器，由前端生成符合 `v2:` 规范的表达式并由后端统一校验。
- 已完成 SQLite 自动化测试和 MySQL 8、PostgreSQL 16 的真实启动迁移验证，并覆盖 Token、缓存、图片 Token、音频时长和视频时长表达式的金额回归。
- 当前 V2 实际接管已支持 `token`、`request`、`image`、`character`、已知上传文件时长的 `audio_duration`、异步任务适配器能在提交前给出已校验时长的 `video_duration`，以及所有表达式引用量都能在预扣前确定的 `mixed`。运行时会分析采购和销售表达式实际引用的变量：引用 `audio_s`/`video_s` 却没有可靠时长时明确拒绝请求；异步视频 `mixed` 仅允许请求次数、视频时长和请求参数等提交前固定量，包含 Token、图片数、音频时长、字符数、请求头或时间函数时明确拒绝。图片数量使用经过 `MaxImageN` 校验并再次封顶的请求值，字符数按 Unicode 字符计算，转录/翻译音频时长由已做饱和转换的文件元数据估算 Token 反算；视频时长再次受 `MaxTaskDurationSeconds` 约束，并把提交时的契约时长、价格版本和候选渠道冻结到预扣及审计快照。异步视频完成后按提交时的固定契约用量结算，避免上游未返回用量时误退费；失败任务仍走原有幂等退款。无法在预扣前可靠确定时长的 TTS 和 `mixed` 不再使用旧计费，而是返回价格契约错误。
- DoubaoVideo/BytePlus 的 Seedance 2.0 Mini 会在适配器校验阶段拒绝 `480p`，包括公共模型名映射到 Mini 上游名的情况；最低分辨率为 `720p`，无效请求不会进入预扣、价格快照创建或上游调用。
- Midjourney 提交和换脸旁路已移除旧按次价格读取，统一按 `request` 用量执行 V2 报价并冻结价格快照；上游成功后完成扣费与结算，上游失败或无需收费的操作归档为 `refunded`，扣费失败进入价格对账。
- 生产部署不属于本地实施步骤；本地运行时仅接受完整 V2 价格链，不自动修改生产状态。
