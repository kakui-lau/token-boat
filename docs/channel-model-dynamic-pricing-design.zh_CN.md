...
- 当前 V2 实际接管已支持 `token`、`request`、`image`、`character` 和已知上传文件时长的 `audio_duration`：图片数量使用经过 `MaxImageN` 校验并再次封顶的请求值，字符数按 Unicode 字符计算，转录/翻译音频时长由已做饱和转换的文件元数据估算 Token 反算；业务用量会同时进入预扣、冻结表达式结算和价格快照。无法在预扣前可靠确定时长的 TTS、`video_duration` 和包含时长的 `mixed` 仍安全回退 legacy；Seedance 按原计划单独启用。
