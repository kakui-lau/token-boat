...
- 当前 V2 实际接管已支持 `token`、`request`、`image` 和 `character`：图片数量使用经过 `MaxImageN` 校验并再次封顶的请求值，字符数按 Unicode 字符计算，业务用量会同时进入预扣、冻结表达式结算和价格快照。`audio_duration`、`video_duration` 和包含时长的 `mixed` 仍强制保持 legacy，避免实际时长适配完成前出现零价预扣；Seedance 按原计划单独启用。
