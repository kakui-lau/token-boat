...
- 独立的熔断分析页已提供当前进程的熔断状态与最近 200 条状态迁移事件，展示渠道名称、连续失败、成功率、平均延迟、冷却截止时间、限流、半开探测和恢复，并支持按渠道和事件类型筛选；Root 管理员可在二次确认后手动清除当前节点的渠道熔断，并写入管理审计和熔断事件。相关接口仅对 Root 管理员开放，不暴露给普通用户。
- 运行时路由按采购成本、成功率、平均延迟和渠道模型质量权重计算综合评分，默认权重分别为 50%、25%、15% 和 10%。成功率使用贝叶斯先验避免冷启动样本抖动，延迟使用 EWMA 平滑；熔断状态仍先于评分执行硬过滤。
