...
16. 官方价版本编辑器按 `billing_mode + price_structure` 自适应：`token + flat` 使用结构化分项表单；其他计费模式、阶梯价和表达式价使用完整配置编辑器。复制历史版本会继承价格分项和计费表达式，不再被 Token 固定价表单阻断；发布能力仍受对应运行时 evaluator 是否启用的约束。
