...
## 86. Playground 账户级调用与 API Key 解耦

截至 2026-09-02，User Console Playground 已移除 API Key 配置，和旧版 Playground 一样按当前登录账户直接测试：

- 页面不再加载或展示 API Key 选择器、Key 环境与 Key 额度，模型范围直接取当前登录用户所属分组；用户只需选择模型即可开始对话；
- 新前端调用不再发送 `api_key_id`。后端沿用既有 Playground 账户级流程，为当前用户构造临时 `playground-<group>` Token 上下文，因此请求仍按用户的钱包或订阅扣费，但不会归属到某一把真实 API Key；
- CopilotKit 适配层将 `apiKeyId` 改为可选，保留对旧调用方显式传入合法 Key ID 的兼容与归属校验，新前端默认不传；
- 本地对话历史升级为 v2，只保存模型、消息和生成参数，不再保存 `apiKeyId` 或分组快照；已有 v1 历史会自动迁移并保留对话内容；
- 回归测试覆盖无 API Key 的消息转发、当前账户分组模型加载、历史迁移以及页面不再出现 Key 选择器。Console 70 个测试文件、379 项测试、TypeScript、生产构建和 Bundle 预算门禁通过，Go 全量测试通过。

本轮没有修改数据库结构、真实 API Key 接口、正式 API 鉴权规则、生产路由，也没有发布或更新线上版本。

## 87. Playground 模型参数面板信息架构

截至 2026-09-02，User Console Playground 参数面板按正式产品设置界面重新整理：

- 参数按“对话指令”和“生成控制”分区，系统提示词增加字符统计，随机性增加精确数值输入、稳定/均衡/创意状态提示和清晰的取值方向；
- 最大输出长度使用带 Token 单位的组合输入框；面板底部固定展示生效范围，并提供恢复默认和完成操作；
- 桌面端侧栏扩展为 512px，窄屏使用完整可用宽度，内容区独立滚动，头部和操作区保持可见；中英文文案同步补齐；
- 新增组件回归测试，覆盖分区信息、精确小数输入、状态变化和恢复默认。Console 71 个测试文件、381 项测试、TypeScript、零警告目标 Lint、生产构建及 Bundle 预算门禁通过。

本轮遵循现有 shadcn/Base UI 的 Field、Card、InputGroup、Slider 与 Sheet 组合规范，没有修改生成参数协议、后端、数据库、生产路由，也没有发布或更新线上版本。
