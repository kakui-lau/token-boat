...
> 当前状态（2026-09-01）：新版报价与计费系统已经完成生产迁移并正式发布。本文第 10 节保留发布前的本地验收记录，当前生产状态以第 11 节为准。
...
- 官方价格或采购价变化时，先生成变更批次和新报价版本，经预览和风险检查后由有权限的管理员发布，不直接覆盖历史价格。
- 每次请求保存采购版本、报价组版本、表达式哈希和最终结算结果，可重放、可审计、可对账。

历史开发阶段约束：

- 功能开发和本地验收阶段不直接修改线上数据；生产迁移与发布随后通过独立发布步骤完成。
...
| `pricing_audit_records` | 发布、生成、绑定、撤销等操作审计 | 按对象和操作记录操作者及时间，不实现双人审批 |
...
8. 管理员预览差异和风险后直接发布；发布采用权限、事务、内容哈希和完整审计防止越权或并发篡改。
...
- [x] 提供风险项和批次详情接口。
- [x] 补充新旧版本逐字段差异预览、风险识别、单人发布和操作审计。
- 验收：同一逻辑模型无论有几个渠道，报价版本中只有一个销售价格项；所有渠道毛利均可预览。

### M3：管理页面

- [x] 新增“销售报价组”页面和菜单。
- [x] 报价组列表支持名称、编码、受众、状态服务端筛选，默认每页 200 条。
...
- [x] 完成生产迁移准备、默认报价初始化和账单关键路径核对。
- [x] 完成一次性线上迁移与发布，新版报价组成为生产单一计费运行时。
...
- [x] 官方价发布自动生成关联采购价草稿，采购价发布自动生成受影响报价组草稿，并展示差异和风险。
...
- [ ] 在 CI 或专用环境补齐 MySQL/PostgreSQL 的持续自动化回归；SQLite 与当前生产数据库发布验收已完成，该项不再表示系统尚未发布。
- [x] 本地端到端验收记录完整。
- [x] 开发阶段没有直接修改线上数据；生产迁移和部署通过独立发布步骤执行。

## 9. 当前基线与已知问题

- 分支：`codex/sales-price-books`。
- 新表、基础生命周期 API、用户解析器、管理页面、采购价目录、单一运行时、模型广场报价和服务测试已经完成。
- 新版报价与计费系统已经部署到生产，旧渠道零售价运行时不再参与生产结算。
- 2026-09-01 复核时，仓库完整 `go test ./...` 已通过；此前记录的 `model/log_format_test.go` 与 OpenAPI 清单基线失败已经不再存在。
- 已发布前端基线通过测试、TypeScript、Oxlint 和生产构建；当前工作区中的后续功能增量与报价系统已发布状态分开管理。

## 10. 2026-08-24 本地验收记录

以下内容是生产发布前的历史快照，不代表当前仍未上线：
...
- 报价组管理：报价组和用户绑定列表改为服务端筛选与分页，默认每页 200 条；绑定记录支持用户名、报价编号和合同编号搜索；报价版本模型项支持带权限校验的 UTF-8 CSV 导出，并对表格公式注入字符做安全转义。
- 自动更新：官方价发布后按既有合同折扣生成采购价草稿；采购价发布后为所有受影响报价组复制当前版本并重算对应模型；操作按触发版本和报价组幂等，可通过重试接口恢复。
- 差异与审计：增加报价版本逐模型参考售价、参考成本、净利率、采购版本和风险差异；增加变更批次列表与详情面板；取消双人审批设计，原审批表迁移并更名为只记录操作事实的定价审计表。
- 当时发布边界：未连接线上数据库，未修改线上配置，未推送、未部署；后续生产发布状态见第 11 节。

## 11. 2026-09-01 生产发布状态

- 新版报价与计费闭环、销售报价组管理、默认报价初始化和数据库迁移器已经随生产版本发布。
- 生产运行时使用“用户绑定报价组 / TOC 默认报价组 + 渠道采购价”的单一路径，不再回退旧渠道零售价。
- 生产公开定价接口 `GET https://tokenboat.com/api/pricing` 已返回结构化 `sales_prices_by_group`、计费组件、单位和最终销售价格；2026-09-01 复核时共返回 40 个模型。
- 报价系统已发布状态与后续模型价格、上下文目录和数据库性能优化分开管理；后续增量未发布不影响本系统的生产完成结论。
