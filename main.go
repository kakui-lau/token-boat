...
func main() {
	...
	go model.StartOptionSyncSubscriber()
	go pricingruntime.StartCatalogSyncSubscriber()
	...
	server.Use(gin.CustomRecovery(func(c *gin.Context, err any) {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": gin.H{
				"message": fmt.Sprintf("Panic detected, error: %v. Please contact TokenBoat support: https://tokenboat.com/support/community-interaction", err),
				"type":    "new_api_panic",
			},
		})
	}))
	...
	if common.DataExportEnabled {
		if err := model.SaveQuotaDataCache(); err != nil {
			common.SysError("failed to save quota data cache during shutdown: " + err.Error())
		}
	}
