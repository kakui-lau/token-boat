...
func TestLogFailedUpstreamRequestPrintsClientAndUpstreamParameters(t *testing.T) {
	...
	assert.Contains(t, logged, `client_request: method=POST url="/pg/chat/completions"`)
	assert.Contains(t, logged, `body={"access_token":"***masked*** [fingerprint=sha256:`)
	assert.Contains(t, logged, `"model":"gpt-5.6-sol"`)
	assert.Contains(t, logged, `upstream_body={"api_key":"***masked*** [fingerprint=sha256:`)
