...
type BillingSettler interface {
	...
	// RefundWithResult completes the refund synchronously when a callback is
	// provided, then reports whether every step succeeded so audit records and
	// the financial operation cannot be separated by a process restart.
