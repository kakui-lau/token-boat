...
func (*recordingBillingSettler) RefundWithResult(_ *gin.Context, completed func(error)) {
	if completed != nil {
		completed(nil)
	}
}
...
func TestPrepareTieredBillingForSelectedGroupRejectsInsufficientWalletTopUp(t *testing.T) {
	...
	const userID = 701
	// Balance does not cover the 50k top-up to the more expensive retry group.
	// TokenBoat does not permit a retry reservation to create a negative wallet.
	...
	// Reservation and wallet remain unchanged when the top-up cannot be funded.
